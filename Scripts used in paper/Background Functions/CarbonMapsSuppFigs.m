...
stack=Snew.spectr;
subdim=ceil(sqrt(length(energy)));
% % % % % if nofig==0&&length(energy)<6  %plotting raw spectra
% % % % %     figure
...
%%% Find particles
meanim = sum(Snew.spectr,3); %meanim=mean(Snew.spectr,3); Old version
...
errprepost = sqrt((Snew.errOD(:,:,preidx)./stack(:,:,postidx)).^2 + ((stack(:,:,preidx).* Snew.errOD(:,:,postidx)./stack(:,:,postidx).^2)).^2); %calculus based (derivative) error propagation formula applied here
pre=stack(:,:,preidx);
...
if ~isempty(sp2idx)
    ...
    % figure,imagesc(doublecarb),colormap gray,colorbar
    doubCarbNois=std(doublecarb(binmap==0)); % calculate the noise of the sp2 peak by
    doublecarb1=doublecarb;
    doublecarb2=doublecarb .* (0.4512/0.8656) ./ (stack(:,:,postidx)-stack(:,:,preidx));
    ...
    % figure,imagesc(doublecarb1),colormap gray,colorbar
    sp2=(doublecarb./carb)  .*  (0.4512/0.8656)   .*spmask; % calculate %sp2 of masked images
